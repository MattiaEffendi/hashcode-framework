...
function calculateScoresV11(&$demons, &$player, $currentTurn, $maxTurns){
    foreach($demons as &$demon){
        $demon->score = min($demon->staminaRecoveredAfter, $player->maxStamina - $player->stamina) / $demon->turnsAfter * pow($maxTurns - $currentTurn - $demon->turnsAfter, 0.2);
    }
}
...
function calculateScoresV21(&$demons, &$player, $currentTurn, $maxTurns){
    foreach($demons as &$demon){
        $partialScore = min($demon->staminaRecoveredAfter, $player->maxStamina - $player->stamina) / $demon->turnsAfter * pow($maxTurns - $currentTurn - $demon->turnsAfter, 0.1);
        if(true){
            $demon->score = $partialScore;
        }
...
while($currentTurn <= $maxTurns){
    ...
    calculateScoresV21($demons, $player, $currentTurn, $maxTurns);
